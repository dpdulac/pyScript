...
from Katana import NodegraphAPI, UI4, ScenegraphManager
...
def createParam(groupNode=NodegraphAPI.GetRootNode()):
    ...
    fstopCameraMetadataParam.setHintString("{'widget': 'checkBox', 'constant': 'True', 'help': 'use the fstop inside the camera metadata'}")
    ...
    # display focus planes
    focusPlanesDisplayParam = groupNode.getParameters().createChildNumber('Display_focus_plane', 1)
    focusPlanesDisplayParam.setHintString("{'widget': 'checkBox', 'constant': 'True', 'help': 'display the focus plane'}")
    ...
    targetConstraintParam.setHintString("{'widget': 'checkBox', 'constant': 'True', 'help': 'check to set an object as target constraint'}")
    ...
    advancedSettigsGroup.setHintString(
        "{'help': 'edit advanced setting like name, color and path of the geometry plane representing the focus plane'}")
    filmAspectRatio = advancedSettigsGroup.createChildNumber('filmAspectRatio', 2.39)
    filmAspectRatio.setHintString("{'help': 'film aspect ratio (i.e 2.35, 1.85...)', 'constant': 'True'}")
    CoCParam = advancedSettigsGroup.createChildNumber('Coc', 0.013)
    CoCParam.setHintString(
        "{'widget': 'popup', 'constant': 'True', 'options': ['0.013', '0.025', '0.035', '0.05'], 'help': 'Circle of confusion (https://improvephotography.com/53601/conquering-the-circle-of-confusion-for-photography/)'}")
    CoCParam.setValue(0.013, 0.0)
    overrideFocalLengthParam = advancedSettigsGroup.createChildNumber('override_focal_length', 0)
    overrideFocalLengthParam.setHintString(
        "{'widget': 'checkBox', 'constant': 'True', 'helpAlert': 'warning', 'help': 'check to override the fov this will not change the fov of the camera but change the calculations of DOF'}")
    focalLenghtParam = advancedSettigsGroup.createChildNumber('focal_length', 50)
    focalLenghtParam.setHintString(
        "{'conditionalVisOps': {'conditionalVisOp': 'equalTo', 'conditionalVisPath': '../override_focal_length', 'conditionalVisValue': '1'}, 'help': 'fov used for calculation of DOF'}")
    # arnold render 3D
    render3DDOFParam = advancedSettigsGroup.createChildNumber('render_3D_DOF', 0)
    render3DDOFParam.setHintString(
        "{'widget': 'boolean', 'constant': 'True', 'help': 'If yes Arnold will render the DOF in 3D and the data will be pushed to the metadata of the primary pass if no only the data will be pushed to the metadata of the primary pass'}")
    ...
    cameraPathGroup.setHintString("{'help': 'camera to use for calculate the DOF', 'constant': 'True'}")
    changeCameraParam = cameraPathGroup.createChildNumber('change_camera', 0)
    changeCameraParam.setHintString("{'widget': 'checkBox', 'constant': 'True', 'help': 'unlock the camera field to change camera'}")
    ...
    geometrySettingsGroup.setHintString(
        "{'help': 'setting for name,color and path of the focus planes and target_focus'}")
    geometryPathParam = geometrySettingsGroup.createChildString('geometry_path', '/root/world/cam/DOF')
    geometryPathParam.setHintString(
        "{'widget': 'scenegraphLocation', 'help': 'path where to put the geometry planes representing the DOF'}")
    ...
    scalePlaneGangParam.setHintString(
        "{'widget': 'checkBox', 'constant': 'True', 'help': 'if on scale all the plane uniformaly if off each plane can be scale independently'}")
    scalePlaneGangParam.setValue(1.0, 0.0)
    scalePlaneParam = geometrySettingsGroup.createChildNumber('scale_focus_planes', 1.0)
    scalePlaneParam.setHintString(
        "{'slider': 'True', 'constant': 'True', 'slidermax': '100.0', 'conditionalVisOps': {'conditionalVisOp': 'equalTo', 'conditionalVisPath': '../scale_plane_gang', 'conditionalVisValue': '1'}, 'help': 'scale all the planes uniformaly'}")
    nearFocusPlaneScaleParam = geometrySettingsGroup.createChildNumber('near_focus_plane_scale', 1.0)
    nearFocusPlaneScaleParam.setHintString(
        "{'slider': 'True', 'constant': 'True', 'slidermax': '100.0', 'conditionalVisOps': {'conditionalVisOp': 'equalTo', 'conditionalVisPath': '../scale_plane_gang', 'conditionalVisValue': '0'}, 'help': 'scale the near focus plane'}")
    focusPlaneScaleParam = geometrySettingsGroup.createChildNumber('focus_plane_scale', 1.0)
    focusPlaneScaleParam.setHintString(
        "{'slider': 'True', 'constant': 'True', 'slidermax': '100.0', 'conditionalVisOps': {'conditionalVisOp': 'equalTo', 'conditionalVisPath': '../scale_plane_gang', 'conditionalVisValue': '0'}, 'help': 'scale the focus plane'}")
    ...
    farFocusPlaneScaleParam.setHintString(
        "{'slider': 'True', 'constant': 'True', 'slidermax': '100.0', 'conditionalVisOps': {'conditionalVisOp': 'equalTo', 'conditionalVisPath': '../scale_plane_gang', 'conditionalVisValue': '0'}, 'help': 'scale the far focus plane'}")
    # target geometry settings
    targetNameParam = geometrySettingsGroup.createChildString('target_name', 'focusTarget')
    targetNameParam.setHintString("{'help': 'name for the target focus ', 'constant': 'True'}")
    ...
    scaleTargetParam.setHintString("{'slider': 'True', 'constant': 'True', 'slidermax': '100.0', 'help': 'scale of target'}")
    # version number
...
def createOpScriptParam(opNode=NodegraphAPI.GetRootNode()):
    ...
    paramDict = {'camPath': {'format': 's',
                             'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.camera_path.camera')"},
                 'method': {'format': 's',
                            'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.method')"},
                 'focusDistance': {'format': 'n',
                                   'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.focus_distance')"},
                 'fstopMetadata': {'format': 'n',
                                   'exp': "getParam(self.getNode().getParent().getName()+'.fstop_camera_metadata')"},
                 'fstop': {'format': 'n', 'exp': "getParam(self.getNode().getParent().getName()+'.fstop')"},
                 'userFocalLength': {'format': 'n',
                                     'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.focal_length')"},
                 'overrideFocalLength': {'format': 'n',
                                         'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.override_focal_length')"},
                 'alembicFile': {'format': 's',
                                 'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.alembic_setting.alembic_path')"},
                 'focusTarget': {'format': 's',
                                 'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'/'+getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.target_name')"},
                 'focusTargetColor': {'format': 'a',
                                      'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.target_color')"},
                 'focusPlanePath': {'format': 's',
                                    'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'/'+getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.focus_plane_name')"},
                 'CoC': {'format': 'n',
                         'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.Coc')"},
                 'nearFocusPlane': {'format': 's',
                                    'exp': 'getParam(self.getNode().getParent().getName()+".advanced_settings.geometry_settings.geometry_path")+"/"+getParam(self.getNode().getParent().getName()+".advanced_settings.geometry_settings.near_focus_plane_name")'},
                 'farFocusPlane': {'format': 's',
                                   'exp': 'getParam(self.getNode().getParent().getName()+".advanced_settings.geometry_settings.geometry_path")+"/"+getParam(self.getNode().getParent().getName()+".advanced_settings.geometry_settings.far_focus_plane_name")'},
                 'targetConstraint': {'format': 'n',
                                      'exp': 'getParam(self.getNode().getParent().getName()+".focus_distance.target_setting.target_constraint")'},
                 'targetConstraintObject': {'format': 's',
                                            'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.target_setting.target_constraint_Object') if getParam(self.getNode().getParent().getName()+'.focus_distance.target_setting.target_constraint_Object') != '' else getParam(self.getNode().getParent().getName()+'.advanced_settings.camera_path.camera')"},
                 'alembicPath': {'format': 's',
                                 'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'/AlembicFocus'"},
                 'alembicAttr': {'format': 's',
                                 'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.alembic_setting.alembic_attr_name')"},
                 'alembicName': {'format': 's',
                                 'exp': "getParam(self.getNode().getParent().getName()+'.focus_distance.alembic_setting.alembic_path')"},
                 'apertureBlades': {'format': 'n',
                                    'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.DOF_settings_3D.aperture_blades')"},
                 'apertureAspectRatio': {'format': 'n',
                                         'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.DOF_settings_3D.aperture_aspect_ratio')"},
                 'apertureBladeCurvature': {'format': 'n',
                                            'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.DOF_settings_3D.aperture_blade_Curvature')"},
                 'apertureRotation': {'format': 'n',
                                      'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.DOF_settings_3D.aperture_rotation')"},
                 'flatFieldFocus': {'format': 'n',
                                    'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.DOF_settings_3D.flat_field_focus')"},
                 'render3D': {'format': 'n',
                              'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.render_3D_DOF')"},
                 'filmAspectRatio': {'format': 'n',
                                     'exp': "getParam(self.getNode().getParent().getName()+'.advanced_settings.filmAspectRatio')"}}
...
def displayFocusNode(dictPlanes=[], display=True):
    sg = ScenegraphManager.getActiveScenegraph()
    locations = []
    for key in dictPlanes.keys():
        locations.append(dictPlanes[key]['location'])
    if display:
        sg.addPinnedLocations(locations, True)
    else:
        sg.removePinnedLocations(locations, True)
...
def createFocusGroup(name='FocusGroup', parent=NodegraphAPI.GetRootNode(), geoPath='/root/world/cam/DOF'):
    ...
    dictPlanes = {'nearFocusPlane': {'nodeName': 'nearFocusPlane', 'location': geoPath + '/near_focus_plane',
                                     'expressionName': expressionBase + 'near_focus_plane_name")',
                                     'expressionScale': expressionScale + 'near_focus_plane_scale")',
                                     'expressionColor': expressionBase + 'near_focus_plane_color")', 'pos': (-200, -10),
                                     'inputMerge': 'i1', 'targetMode': False},
                  'focusPlane': {'nodeName': 'focusPlane', 'location': geoPath + '/focus_plane',
                                 'expressionName': expressionBase + 'focus_plane_name")',
                                 'expressionScale': expressionScale + 'focus_plane_scale")',
                                 'expressionColor': expressionBase + 'focus_plane_color")', 'pos': (0, -10),
                                 'inputMerge': 'i2', 'targetMode': False},
                  'farFocusPlane': {'nodeName': 'farFocusPlane', 'location': geoPath + '/far_focus_plane',
                                    'expressionName': expressionBase + 'far_focus_plane_name")',
                                    'expressionScale': expressionScale + 'far_focus_plane_scale")',
                                    'expressionColor': expressionBase + 'far_focus_plane_color")', 'pos': (200, -10),
                                    'inputMerge': 'i3', 'targetMode': False},
                  'focusTarget': {'nodeName': 'focusTarget', 'location': geoPath + '/focus_target',
                                  'expressionName': expressionBase + 'target_name")',
                                  'expressionScale': expressionBase + 'scale_target")',
                                  'expressionColor': expressionBase + 'target_color")', 'pos': (200, -10),
                                  'inputMerge': 'i4', 'targetMode': True}}

    for key in dictPlanes.keys():
        posY -= 100.0
        # create the 3 planes
        plane = createPlane(nodeName=dictPlanes[key]['nodeName'], location=dictPlanes[key]['location'],
                            rootNode=groupNode, targetMode=dictPlanes[key]['targetMode'])
        # set the expression
        ...
        plane.getParameter('location').setExpression(
            expressionBase + 'geometry_path")' + '+"/"+' + dictPlanes[key]['expressionName'], True)
        if dictPlanes[key]['targetMode']:
            plane.getParameter('disable').setExpression(
                "0 if getParam(self.getNode().getParent().getName()+'.focus_distance.method') == '0' else 1", True)
        else:
            plane.getParameter('disable').setExpression(
                "0 if getParam(self.getNode().getParent().getName()+'.Display_focus_plane') == 1 else 1", True)
    ...
    transFormNode.getParameter('disable').setExpression(
        "0 if getParam(self.getNode().getParent().getName()+'.focus_distance.method') == '0' and getParam(self.getNode().getParent().getName()+'.focus_distance.target_setting.target_constraint') == 0 else 1",
        True)
    transFormNode.getParameter('path').setExpression(
        "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'/'+getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.target_name')",
        True)
    posY -= 100.0
    ...
    alembicNode.getParameter('disable').setExpression(
        "0 if getParam(self.getNode().getParent().getName()+'.focus_distance.method') == '1' and getParam(self.getNode().getParent().getName()+'.focus_distance.alembic_setting.alembic_path') != '' else 1",
        True)
    alembicNode.getParameter('name').setExpression(
        "getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'/AlembicFocus'",
        True)
    alembicNode.getParameter('abcAsset').setExpression(
        "getParam(self.getNode().getParent().getName()+'.focus_distance.alembic_setting.alembic_path')", True)
    posX = 100.0
    ...
    opScriptDOFNode.getParameter('CEL').setExpression(
        "'( /' + getParam(self.getNode().getParent().getName()+'.advanced_settings.geometry_settings.geometry_path')+'//* + /root)'",
        True)
    ...
    # display the focus plane
    sg = ScenegraphManager.getActiveScenegraph()
    sg.addPinnedLocation(geoPath)
    for key in dictPlanes.keys():
        sg.addPinnedLocation(geoPath+'/'+key)
