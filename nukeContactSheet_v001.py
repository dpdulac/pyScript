#!/usr/bin/env python
# coding:utf-8
""":mod:`nukeContactSheet_v001` -- dummy module
===================================
...
def findShots(taskname='compo_precomp', seq='s0300', shotList=['Not']):
    if shotList[0] == 'Not':
    ...
    for v in sg.find('PublishedFile', filters, ['code', 'entity', 'version_number', 'path', 'published_file_type', 'entity.Shot.sg_cut_in','entity.Shot.sg_cut_out','task'], order=[{'field_name':'created_at','direction':'desc'}]):
        ...
        if not entityName in res:
            res[entityName] = {v['published_file_type']['name']:[v]}
        continue
...
def createContactSheet(shotFrame = 'start',nbColumns = 6,seq ="s0300",taskname='compo_precomp'):
    ...
    for entityName, fileTypes in res.items():
        ...
        for fileType, versions in fileTypes.items():
            ...
            rootFrame = versions[0]['path']['local_path'][:versions[0]['path']['local_path'].find('%04d')]
            rootDir = rootFrame[:rootFrame.rfind("/")]
            #if the first frame is chosen for contactsheet
            if shotFrame == 'mid':
                #take the cut_in frame from shotgun data
                frame = str(int((versions[0]['entity.Shot.sg_cut_in'] + versions[0]['entity.Shot.sg_cut_out'])/2)).zfill(4)+'.exr'
                ...
                frame = str(versions[0]['entity.Shot.sg_cut_out']).zfill(4)+'.exr'
                tmpFullFile = rootFrame+frame
                #check if the frame exist if not go in the directory and chose the closest frame to the chosen frame in the directory
                fullFile = isFrameGood(fileIn = tmpFullFile,dirIn = rootDir)["filePath"]
                #put the chosen frame in the dictionary to be chosen as a contactsheet frame
                ...
                frame = str(versions[0]['entity.Shot.sg_cut_in']).zfill(4)+'.exr'
                tmpFullFile = rootFrame+frame
                #check if the frame exist if not go in the directory and chose the closest frame to the chosen frame in the directory
                fullFile = isFrameGood(fileIn = tmpFullFile,dirIn = rootDir)["filePath"]
                #put the chosen frame in the dictionary to be chosen as a contactsheet frame
            ...
            shotPath[entityName]['allFrames']= versions[0]['path']['local_path'][:versions[0]['path']['local_path'].find('%04d')]+"""####.exr"""
            shotPath[entityName]['rootFrame']= rootFrame
            shotPath[entityName]['version'] = str(versions[0]['version_number'])
            ...
            checkFrame = shotPath[entityName]['rootFrame']+str(cut_in).zfill(4)+".exr"
            ...
            checkFrame = shotPath[entityName]['rootFrame']+str(cut_out).zfill(4)+".exr"
            ...
            cut_mid = int((versions[0]['entity.Shot.sg_cut_in'] + versions[0]['entity.Shot.sg_cut_out'])/2)
            checkFrame = shotPath[entityName]['rootFrame']+str(cut_mid).zfill(4)+".exr"
