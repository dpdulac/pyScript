...
def getNewVersionNumber(path = '/tmp', type = 'jpg'):
    ...
    if len(movieFile) == 0:
        ...
        newNumber = int(lastFile[start:end]) +1
        return "_v"+ str(newNumber).zfill(3)

#check if directory exist and return the proper path for the file in the writes nodes
def createFileOutput(path = "/tmp", seq = "s1300", masterName='',createMasterDir=False):
    if not createMasterDir:
        ...
        JPG = path+'/'+seq+"/Masters/"+masterName+"/JPG/"
        checkDir(JPG)
        version = getNewVersionNumber(path=JPG,type = 'jpg')
        JPG = path+'/'+seq+"/Masters/"+masterName+"/JPG/"+masterName+version+".jpg"

        EXR = path+'/'+seq+"/Masters/"+masterName+"/EXR/"
        checkDir(EXR)
        version = getNewVersionNumber(path=EXR,type = 'exr')
        EXR = path+'/'+seq+"/Masters/"+masterName+"/EXR/"+masterName+version+".exr"

        HD = path+'/'+seq+"/Masters/"+masterName+"/HD/"
        checkDir(HD)
        version = getNewVersionNumber(path=EXR,type = 'jpg')
        HD = path+'/'+seq+"/Masters/"+masterName+"/HD/"+masterName+version+"HD.jpg"

        Movie = path+'/'+seq+"/Masters/"+masterName+"/Movie/"
        checkDir(Movie)
        version = getNewVersionNumber(path=EXR,type = 'mov')
        Movie = path+'/'+seq+"/Masters/"+masterName+"/Movie/"+masterName+version+".mov"
...
def createContactSheetAllSequence(shotFrame = 'start',nbColumns = 6,seq ="s0300",taskname='compo_precomp',lightOnly = False, render=True,display = False):
    import nuke
    import nuke.rotopaint as rp
    root = nuke.root()
    root['format'].setValue( 'HD_1080' )
...
def get_args():
    ...
    displayOn = args.d
    return seqNumber, frameChosen, task, lightOn, masterNumber, masterOn, renderOn, mastersInSeq, shotNumber, displayOn


def main():
    seqNumber, frameChosen, task, lightOn, masterNumber, masterOn, renderOn,mastersInSeq,shotNumber, displayOn = get_args()
    if mastersInSeq:
        ...
        if masterOn:
            ...
            createContactSheetAllSequence(shotFrame = frameChosen,nbColumns = 6,seq =seqNumber,taskname=task,lightOnly = lightOn,render=renderOn, display = displayOn)
...
#createContactSheetAllSequence('mid',nbColumns = 6,seq ="s0750",taskname='compo_precomp',lightOnly = False)
#createContactSheetMasters(shotFrame = 'start',nbColumns = 6,seq ="s0300",taskname='compo_precomp',lightOnly = False, master='All')
#test(shotFrame = 'start',nbColumns = 6,seq ="s0300",taskname='compo_precomp',lightOnly = False, master='All')
