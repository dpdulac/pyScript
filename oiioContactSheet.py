...
_TASKLIST_ = ['compo_stereo', 'compo_comp', 'light_precomp', 'light_prelight', 'confo_render', 'anim_master',
              'confo_anim', 'anim_main', 'layout_base']
...
def findShots(taskname='compo_comp', seq='p00300', shotList=[]):
    ...
    while shotList != [] and switchTask < len(_TASKLIST_):
        ...
        for v in sg.find('PublishedFile', filters,
                         ['code', 'entity', 'entity.Shot.sg_status_list', 'entity.Shot.sg_cut_order', 'version_number',
                          'path', 'published_file_type', 'entity.Shot.sg_cut_in', 'entity.Shot.sg_cut_out', 'task',
                          'entity.Shot.sg_sequence', 'entity.Shot.sg_frames_of_interest'],
...
def findShotsInList(seq='s1300', shotList=[], taskname='compo_comp'):
    ...
    for shot in shotList:
        ...
        while found == False:
            ...
            fileFound = sg.find('PublishedFile', filters,
                                ['code', 'entity', 'entity.Shot.sg_status_list', 'entity.Shot.sg_cut_order',
                                 'version_number', 'path', 'published_file_type', 'entity.Shot.sg_cut_in',
                                 'entity.Shot.sg_cut_out', 'task', 'entity.Shot.sg_sequence',
                                 'entity.Shot.sg_frames_of_interest'],
                                order=[{'field_name': 'version_number', 'direction': 'desc'}])
            if fileFound == []:
                switchTask = switchTask + 1
            else:
...
def contactSheet(task='compo_comp', seq='s0180', res={}, format='jpg', scale='full', shotgunData=True,
                 printFormat=False, nrow=5, pdf=True):
        ...
        for shot in cutOrderSeq:
            ...
            if imMod != 0:
                if not contactsheet in dictListShot:
                    ...
                    dictListShot[contactsheet]['imagePath'] = path + 'contactSheet_print_' + seq + '_' + str(
                        nbPage).zfill(3) + '.' + format
        ...
        for contactSheet in sorted(dictListShot.keys()):
            if contactSheet == 'contact1':
                dictListShot[contactSheet]['masterBuf'], masterBufwidth, masterBufHeight = createMasterBuf(
                    ...
                    bottomBar=True)
                oiio.ImageBufAlgo.paste(dictListShot[contactSheet]['masterBuf'], (masterBufwidth / 2) - (widthLogo / 2),
                                        60, 0, 0, logo)
                # adding page number
                ...
                dictListShot[contactSheet]['masterBuf'], averageList = placeImages(res=res, task=task, cutOrderSeq=
                dictListShot[contactSheet]['listShot'], imgh=578 + (2 * maxheight),
...
def createMasterBuf(maxwidth=2048, maxheight=858, printFormat=True, nrow=5, ncol=13, space=40, seq='s0265',
                    task='compo_comp', topBar=True, bottomBar=True):
...
def placeImages(res={}, shotgunData=True, task='compo_comp', cutOrderSeq=[], imgh=2294, masterBuf=oiio.ImageBuf(),
                printFormat=False, ncol=13, nrow=5, space=40, maxwidth=2048, maxheight=858, averageList=[]):
    offsetwidth = 0
    offsetheight = 0
    nbimage = 0
...
def get_args():
    ...
    parser.add_argument('--x', '-x', action='store_true',
                        help='create the contactSheet without opening the GUI if this arguments is not present or no sequences are pass, the GUI will open')
    parser.add_argument('--t', '-t', type=str,
                        help='task name: ' + str(_TASKLIST_) + '\nIf no task is chosen, the default is: comp_precomp')
    parser.add_argument('--f', '-f', type=str, help='format for the output image the supported format are: ' + str(
        _OUTPUTFORMAT_) + ' with the default being jpg ')
    parser.add_argument('--nd', '-nd', action='store_false', help='do not display metadata')
    parser.add_argument('--s', '-s', type=str,
                        help='output image size the argument are "full", "half", "quarter"\nIf no scale is chosen, the default is: quarter')
    parser.add_argument('--pf', '-pf', action='store_true', help='output image in print format (A4,A3)')
    parser.add_argument('--ncol', '-ncol', type=int,
                        help='number of column for the contactSheet(do not apply in print format)')
...
def main():
    ...
    for seq in sequences:
        ...
        imageList.append(contactSheet(task, seq, res, format, scale=outImSize, printFormat=printFormat, nrow=nrow,
                                      shotgunData=noMeta))
    if nrv:
        print 'serving in rv'
        playInRv(imageList)
