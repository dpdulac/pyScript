...
#from rv import rvui, commands, extra_commands, runtime
...
def test(seq='s0300'):
    ...
    sg_filter = [['project', 'is', project],
                 #['content','is',task],
                 ['code', 'is', seq]]
    entity = sgw.sg_find('Sequence', sg_filter, ['shots'])
    print entity[0]['shots']
    ...
    if not taskSG:
        print 'No task %s exists in Sequence %s !' % ('confo_layout', entity[0]['shots'])
        return 0
...
def testArg():
    ...
    seqNumber = "s"+seqNumber
    if not shotNumber or len(shotNumber) == 0:
        listShotName = seqShotList(seq='s0300')
    else:
        for i in shotNumber:
            listShotName.append(seqNumber+"_p"+ i.zfill(5))
    print listShotName
